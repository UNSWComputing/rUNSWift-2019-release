...
void populate_options(po::options_description &config_file_options) {
   ...
   simulation_config.add_options()
      ...
      ("simulation.record_ground_truth", po::value<bool>()->default_value(false),
       "Whether to store in a file the robot pose ground-truth data sent from rcssserver3d in a file")
      ("simulation.ground_truth_record_path",
      po::value<string>()->default_value("simulation-ground-truth-record.txt"),
       "Where to save the ground-truth pose received from the simulator")
...
po::options_description store_and_notify(vector<string> argv,
   ...
   if (vm.count("simulation")) {
      ifs.open("/home/nao/data/simswift.cfg");
      store(parse_config_file(ifs, config_file_options), vm);
      ifs.close();
   }
...
#ifdef CTC_2_1
   ifs.open("/home/nao/data/runswiftv5.cfg");
   store(parse_config_file(ifs, config_file_options), vm);
   ifs.close();
#endif
