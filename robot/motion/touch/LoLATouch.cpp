...
static void load(const vector<msgpack::object> &vector, float *array) {
   transform(vector.begin(), vector.end(), array, bind(&msgpack::object::as<float>, placeholders::_1));
}
...
// Jayen's not a big fan of making system calls (SAY, llog, ...) in this thread
// Consider moving this to perception or another low-priority thread
static inline ButtonPresses doButtons(bool chest, bool left, bool right, bool falling) {
...
void LoLATouch::loadCache() {
   const map<string, vector<msgpack::object> > sensorMap = read();
   if (sensorMap.size() == 0)
      return;
