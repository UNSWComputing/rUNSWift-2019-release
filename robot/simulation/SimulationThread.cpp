    ...
    , ticks_since_connected_(0)
{
    ...
    float initial_theta;
    if ((bb->config)["stateestimation.initial_pose_type"].as<string>() == "SPECIFIED"){
    ...
    if ((bb->config)["simulation.record_ground_truth"].as<bool>())
    {
        ground_truth_record_stream_.open((bb->config)["simulation.ground_truth_record_path"].as<string>().c_str());
    }
}

SimulationThread::~SimulationThread()
{
    if ((blackboard->config)["simulation.record_ground_truth"].as<bool>())
    {
        ground_truth_record_stream_.close();
    }
}
...
void SimulationThread::tick()
{
    ...
    ++ticks_since_connected_;

    // Write to file, true x,y,theta of robot
    if ((blackboard->config)["simulation.record_ground_truth"].as<bool>() && recv.parsedMyPosAndOrien)
    {
        ground_truth_record_stream_ << recv.myPos[0] << " " << recv.myPos[1] << " " << recv.myOrien << endl;
    }
    // cout << "ticks_since_connected_" << ticks_since_connected_ << endl;
    // cout << sim_timer_.elapsed_us() << endl;
}
...
bool SimulationThread::shouldRestart(SensorValues& s)
{
    ...
    {
        bool fallen = abs(RAD2DEG(s.sensors[::Sensors::InertialSensor_AngleX])) > 70 || abs(RAD2DEG(s.sensors[::Sensors::InertialSensor_AngleY])) > 70;

        // we check for 10 ticks, since angle values seem to be crazy when the robot is first launched
        return fallen && ticks_since_connected_ > 30;
    }
    else
      return false;
}
