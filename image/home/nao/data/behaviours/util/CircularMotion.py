...
def left(radius_mm, tangent_vel_mm_s):
    return (tangent_vel_mm_s *
            tan(angular_velocity(radius_mm, tangent_vel_mm_s)))
...
def tangent_heading(robot_pos, centre_pos, clockwise=True):
    ...
    return normalisedTheta(
        vec_centre_to_robot.heading() + pi / 2 * (-1 if clockwise else 1))
...
def distance_from_r1t1_to_r2t2(r1_mm, r2_mm, theta1_rad, theta2_rad):
    ...
    circumference = (r1_mm * theta2_rad +
                     r2_mm * theta2_rad -
                     r1_mm * theta1_rad -
                     r2_mm * theta1_rad) / 2

    d = sqrt((r2_mm - r1_mm) * (r2_mm - r1_mm) +
...
def distance_from_r1_to_r2_delta_theta(
        r1_mm, r2_mm, delta_theta_rad):
    theta1_rad = 0
    theta2_rad = delta_theta_rad
    return distance_from_r1t1_to_r2t2(r1_mm, r2_mm, theta1_rad,
                                      theta2_rad)
